...
class NutritionDetail(BaseModel):
    ...
    calories: float = 0; protein_g: float = Field(default=0, alias="proteinG"); fat_g: float = Field(default=0, alias="fatG"); carbohydrate_g: float = Field(default=0, alias="carbohydrateG")
class DietRecord(BaseModel):
    model_config = ConfigDict(populate_by_name=True)
    id: str; dish_id: str | None = Field(default=None, alias="dishId"); food_name: str | None = Field(default=None, alias="foodName"); food_nutrition_tags: list[str] = Field(default_factory=list, alias="foodNutritionTags"); meal_time: datetime | None = Field(default=None, alias="mealTime"); quantity: float = 1; dish_nutrition_detail: NutritionDetail = Field(default_factory=NutritionDetail, alias="dishNutritionDetail")
class NutritionTagCount(BaseModel):
    model_config = ConfigDict(populate_by_name=True)
    category_id: str | None = None; nutrition_tag: str; count: int = 0
class RecommendationCategory(BaseModel):
    model_config = ConfigDict(populate_by_name=True)
    category_id: str; category_name: str; nutrition_tags: list[str] = Field(default_factory=list)
...
class AnalysisResult(BaseModel):
    health_score: float = Field(ge=0, le=100, alias="healthScore"); risk_summary: str = Field(max_length=1000, alias="riskSummary"); optimization_suggestion: str = Field(max_length=2000, alias="optimizationSuggestion"); recommendations: list[Recommendation] = Field(default_factory=list)
    @field_validator("recommendations")
    @classmethod
    def unique_categories(cls, value):
        ids = [x.category_id for x in value]
        if len(ids) != len(set(ids)): raise ValueError("recommendation categories must be unique")
        return value
