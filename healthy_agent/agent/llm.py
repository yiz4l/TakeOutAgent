...
class AnalysisChain:
        ...
        if settings.enable_llm:
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        "你是健康饮食分析助手。只能基于 AUTHORITY_FACTS 和 RULE_SUMMARY 作事实判断。不得修改、猜测或覆盖 Java 业务事实；冲突时以 AUTHORITY_FACTS 为准。输出健康建议，不进行疾病诊断。",
                    ),
                    (
                        "human",
                        "AUTHORITY_FACTS:\n{facts}\nRULE_SUMMARY:\n{rules}\nUSER_MEMORY:\n{memory}\n请给出结构化健康分析。",
    ...
    def invoke(self, context: HealthAnalysisContext, rules: str, memory: str):
        ...
        return self.chain.invoke(
            {
                "facts": context.model_dump_json(by_alias=True),
                "rules": rules,
                "memory": memory,
            }
        )
...
class PeriodSummaryChain:
    ...
    def __init__(self, settings: Settings):
        ...
        if settings.enable_llm:
            ...
            ).with_structured_output(PeriodSummary)
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        "你负责把结构化饮食趋势压缩成周期总结。只能根据输入事实，不要编造数值。",
                    ),
                    (
                        "human",
                        "周期类型={period_type}\n输入事实={facts}\n请输出结构化周期总结。",
    ...
    def invoke(self, period_type: str, facts: str):
        return (
            self.chain.invoke({"period_type": period_type, "facts": facts})
            if self.chain
            else None
        )
