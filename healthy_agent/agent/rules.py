...
def calculate(context: HealthAnalysisContext) -> RuleSummary:
    ...
    for record in context.diet_records:
        for key in totals:
            totals[key] += getattr(record.dish_nutrition_detail, key) * max(record.quantity, 0)
    ...
    factor = {"MUSCLE_GAIN": 1.6, "FAT_LOSS": 1.2, "MAINTAIN": 0.8}.get(target_type, 0.8)
    protein_target = round(weight * factor, 1)
    recent = context.recent_daily_nutrition
    recent_average = round(sum(day.protein_g for day in recent) / len(recent), 1) if recent else 0.0
    deficit_days = sum(day.protein_g < protein_target for day in recent)
    blocked = tuple(dict.fromkeys((profile.allergies + profile.disliked_foods) if profile else []))
    return RuleSummary(
        **totals,
...
def build_result(context: HealthAnalysisContext, summary: RuleSummary):
    ...
    if summary.recent_protein_deficit_days >= 7:
        score -= 10
        risks.append(f"最近{len(context.recent_daily_nutrition)}天有{summary.recent_protein_deficit_days}天蛋白质不足")
    if summary.calories == 0:
        score -= 10
        risks.append("当天暂无可分析的饮食记录")
    suggestion = (
        "建议下一餐补充符合个人忌口和过敏限制的优质蛋白质，并搭配蔬菜和主食。"
        if summary.protein_deficit else "当前蛋白质达到规则目标，后续继续保持饮食多样性。"
    )
    blocked = tuple(item.lower() for item in summary.blocked_foods)
    recommendations = []
    for category in context.available_recommendation_categories:
        searchable = f"{category.category_name} {' '.join(category.nutrition_tags)}".lower()
    ...
    return max(score, 0), "；".join(risks) or "暂无明显风险", suggestion, recommendations[:3]
