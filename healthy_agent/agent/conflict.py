...
def resolve(
    result: AnalysisResult, context: HealthAnalysisContext, fallback: AnalysisResult
) -> AnalysisResult:
    ...
    categories = {
        item.category_id: item for item in context.available_recommendation_categories
    }
    profile = context.health_profile
    blocked = [
        item.lower()
        for item in ((profile.allergies + profile.disliked_foods) if profile else [])
    ]
    recommendations = []
    for item in result.recommendations:
        ...
        if item.category_id in allowed and not any(
            food in searchable for food in blocked
        ):
